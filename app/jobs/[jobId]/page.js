...
const JobIdPage = () => {
  ...
  const [editingContent, setEditingContent] = useState({}); // stores edited content per app
  const [openAppId, setOpenAppId] = useState(null); // which app accordion is open
  ...
  // Update job application content
  const updateJobApp = async (index) => {
    ...
    if (updatedContent === undefined || updatedContent === jobApps[index].content) return; // no change
  ...
  return (
    <div className="relative flex justify-around min-h-screen p-4 items-center">
      {/* Spinner overlay */}
      {loading && (
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col items-center">
                <LiquidButton
                  ...
                  className={`text-xl py-3 px-6 font-bold rounded-full mt-2 
                    ${!profileComplete ? "opacity-50 cursor-not-allowed" : "hover:scale-105"}`}
        ...
        {jobApps.length === 0 ? (
          ...
          jobApps.map((app, index) => (
            <div key={index} className="mb-4">
              <LiquidButton
                className={`w-full text-left font-bold text-lg py-2 mb-1 ${openAppId === index ? "bg-blue-600" : "bg-white/10 text-black"}`}
              ...
              <AnimatePresence>
                {openAppId === index && (
                  <motion.div
                    ...
                    className="bg-white/5 p-4 rounded-md text-sm"
                  >
                    <textarea
                      ...
                      className="w-2xl p-2 text-sm rounded-md bg-white/10 resize-none"
                    />
                  </motion.div>
                )}
              </AnimatePresence>

...
const JobIdPage = () => {
  ...
  const fetchJob = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/user/jobs/${jobId}`,
        {
          headers: {
  ...
  const fetchJobApps = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/user/jobapp/${jobId}`,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Failed to fetch jobApps with status ${response.status}`
        );
    } catch (err) {
      console.error(err);
      setError(err.message || "Network error");
  ...
  const fetchProfileStatus = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/user/profile/status`,
  ...
  const generateJobApp = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Failed to create job application with status ${response.status}`
        );
    } catch (err) {
      console.error(err);
      setError(err.message || "Network error");
  ...
  const updateJobApp = async (index) => {
    ...
    if (
      ...
      updatedContent === jobApps[index].content
    )
      return;

    try {
      const response = await fetch(
        `http://localhost:8080/api/user/jobapp/${appId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${jwtToken}`,
      ...
      if (response.ok) {
        setJobApps((prev) =>
          prev.map((app, i) =>
            i === index ? { ...app, content: updatedContent } : app
          )
        );
  ...
  useEffect(() => {
    if (jobId) fetchJobApps();
  }, [jobId]);

  return (
    <div className="relative flex flex-col pt-24 justify-around min-h-screen p-4 items-center text-[var(--foreground)] bg-[var(--background)]">
      {loading && (
      ...
      {error && (
        <p className="text-red-500 absolute top-4 left-1/2 -translate-x-1/2">
          {error}
        </p>
      )}
      ...
      <div className="flex-1 text-center items-center">
        {job ? (
          <>
            <h1 className="text-3xl font-black pb-4">
              {job.jobTitle}
            </h1>
            ...
            <div className="text-md max-w-[80%] whitespace-pre-wrap break-words mx-auto p-2">
              {job.description}
            </div>
            ...
            <div className="flex flex-col gap-2 pt-4">
              <div className="flex flex-col items-center pt-2">
                ...
                <LiquidButton
                  ...
                  className={`text-xl py-3 px-6 font-bold rounded-full mb-4 ${
                    !profileComplete
      ...
      <div className="flex flex-col max-w-3xl text-center">
        ...
        {jobApps.length === 0 ? (
          <p>You haven&apos;t generated any applications for this job yet.</p>
        ) : (
          jobApps.map((app, index) => (
            <div key={index} className="mb-4">
              <LiquidButton
                className={`w-full text-left font-bold text-lg py-2 mb-1 ${
                  openAppId === index
                    ? "bg-[var(--primary)]"
                    : "bg-[var(--muted)]/20"
                }`}
              ...
              <AnimatePresence>
                {openAppId === index && (
                  <motion.div
                    ...
                    className="bg-[var(--muted)]/20 p-4 rounded-lg 
             w-[900px] max-w-full ml-auto mr-auto"
                  >
                    <textarea
                      rows={Math.max(12, app.content.split("\n").length)}

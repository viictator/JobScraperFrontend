...
// The import for StartScraper has been removed to resolve the compilation error.
...
export default function Home() {
    ...
    useEffect(() => {
        // By adding credentials: 'include', the browser will attach the JSESSIONID 
        // cookie (set by Spring Boot upon successful login) to this request.
        fetch('http://localhost:8080/api/jobs', {
            credentials: 'include', // CRITICAL for cookie-based session authentication
        })
          .then((res) => {
            if (!res.ok) {
                // If status is 401, it means the session cookie was missing or expired.
                if (res.status === 401 || res.status === 403) {
                    throw new Error('You must be logged in to view job listings.');
                }
                throw new Error('Start the backend');
            }
            return res.json();
          })
          .then((data) => {
    ...
    return (
        <main className="min-h-screen bg-[#0f0f0f] px-6 py-12 flex flex-col items-center">
            ...
            {/* Removed the <StartScraper /> component to resolve the unresolved import error. */}
